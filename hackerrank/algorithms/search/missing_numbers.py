...
n_size = int(input())
list_a = [int(p) for p in input().split()]
m_size = int(input())
list_b = [int(p) for p in input().split()]

left_items = Counter(list_a)

missing_items = OrderedSet()
for original_item in list_b:
    if original_item in left_items:
        left_items[original_item] -= 1
        if left_items[original_item] == 0:
            del left_items[original_item]
    else:
        missing_items.add(original_item)

print(' '.join((str(item) for item in missing_items)))
